...
| 📈 | **数据可视化** — 跨项目仪表盘：信号时间线、渠道分布、类型分布、可信度漏斗，一眼看清你的记录节奏。 | |
...
<img src="docs/screenshots/data-visualization.png" width="820" alt="数据可视化仪表盘">

信号时间线、渠道分布、类型分布、可信度漏斗——四张图跨项目聚合，不需要切换来切换去。渠道断了几天、哪类信号最多、可信度分布如何，一眼看出。
...
| 📈 | **数据可视化** | 跨项目仪表盘：信号时间线、渠道分布、类型分布、可信度漏斗。 | |
...
- **信号时间线** — 最近 30 天每天新增信号数。记录断档在哪几天，一眼看出。
- **渠道分布** — 信号来自哪些渠道，占比多少。
- **信号类型分布** — 浏览 / 注册 / 回复 / 付费 / 流失各有多少条。
- **可信度漏斗** — 全部信号 → 中/高可信 → 高可信，看你的信号质量。
